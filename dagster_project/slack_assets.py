        ...
        try:
            ...
            while True:
                if config and messages_processed >= config.max_messages_per_channel:
                    self.logger.info(f"Reached max messages limit ({config.max_messages_per_channel}) for channel {channel_id}")
                    break
                ...
                for message in batch_messages:
                    ...
                    if config and config.include_reactions:
                        try:
                            ...
                            self.logger.warning(f"Could not get reactions for message {message['ts']}: {e}")
                            message['reactions'] = []
                    ...
                    if config and config.include_threads and message.get('thread_ts'):
                        try:
                            ...
                            self.logger.warning(f"Could not get thread replies for message {message['ts']}: {e}")
            ...
            with self.neo4j_driver.session() as session:
                ...
                if relationships:
                    for rel in relationships:
                        ...
                        self.logger.info(f"Created relationship {rel['type']} between {rel['source_type']} {rel['source_id']} and {rel['target_type']} {rel['target_id']}")
...
def scrape_slack_content(context: AssetExecutionContext, config: SlackConfig) -> Dict[str, Any]:
    ...
    indices = ['slack_channels', 'slack_messages', 'slack_files', 'slack_canvases', 'slack_pins', 'slack_bookmarks']
